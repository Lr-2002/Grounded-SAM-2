...
sample_points_from_masks = track_utils.sample_points_from_masks
...
def update_bboxes(bboxes, labels, confidences, image_width, image_height):
    ...
    for i, bbox in enumerate(bboxes):
        x1, y1, x2, y2 = bbox
        if (
            x1 <= width_threshold  # Close to left border
            or x2 >= (image_width - width_threshold)  # Close to right border
            or y1 <= height_threshold  # Close to top border
            or y2 >= (image_height - height_threshold)  # Close to bottom border
        ):
            agents.append(i)
...
def filte_big_box(boxes, labels, confidences, total_area, w, h):
    ...
    filtered_boxes, filtered_labels, filtered_confid, remain, num_len = update_bboxes(
        filtered_boxes, filtered_labels, filtered_confid, w, h
    )
...
class VideoProcessor:
    ...
    def split_video(self):
        ...
        frame_generator = sv.get_video_frames_generator(
            self.video_path, stride=1, start=0, end=None
        )
    ...
    def check_if_need_split(self):
        if self.re_split:
            ...
            if not os.path.exists(self.source_video_frame_dir):
                ...
                os.path.exists(self.source_video_frame_dir)
                and len(os.listdir(self.source_video_frame_dir)) == 0
            ):
                self.split_video()
    ...
    def load_frame_name(self):
        ...
        self.inference_state = self.video_predictor.init_state(
            video_path=self.source_video_frame_dir
        )
    ...
    def set_better_ref(self, stride=3):
        ...
        for idx in range(0, self.video_len, stride):
            ...
            img, input_boxes, class_names, confidences = self.gdino_inner_process(
                img_path
            )
            w, h = self.video_info.resolution_wh
            input_boxes, class_names, confidences, remain, num_len = filte_big_box(
                input_boxes, class_names, confidences, w * h, w, h
            )
    ...
    def load_converted_image(self):
        img_path = os.path.join(
            self.source_video_frame_dir, self.frame_names[self.ann_frame_idx]
        )
    ...
    def gdino_process(self):
        ...
        image, input_boxes, confidences, class_names = self.gdino_inner_process(
            img_path
        )

        converted_image = np.array(image.convert("RGB"))
        h, w = converted_image.shape[:2]
        self.input_boxes, self.class_names, self.confidences, _, _ = filte_big_box(
            input_boxes, class_names, confidences, w * h, w, h
        )
    ...
    def sam2_image_process(self):
        ...
        self.image_masks, self.image_scores, self.image_logits = (
    ...
    def prepare_sam2_video(self):
        ...
        assert self.prompt_type_for_video in [
            "point",
            "box",
            "mask",
        ...
        if self.prompt_type_for_video == "point":
            ...
            all_sample_points = sample_points_from_masks(
                masks=self.image_masks, num_points=10
            )

            for object_id, (label, points) in enumerate(
                zip(self.objects, all_sample_points), start=1
            ):
        ...
        # Using box prompt
            ...
            for object_id, (label, box) in enumerate(
                zip(self.objects, self.input_boxes), start=1
            ):
                _, out_obj_ids, out_mask_logits = (
                    self.video_predictor.add_new_points_or_box(
                        inference_state=self.inference_state,
        ...
        # Using mask prompt is a more straightforward way
            ...
            for object_id, (label, mask) in enumerate(
                zip(self.objects, self.image_masks), start=1
            ):
        ...
        for frame_idx, segments in self.video_segments.items():
            ...
            if save_all:
                ...
                if self.save_video:
                    img = cv2.imread(
                        os.path.join(
                            self.source_video_frame_dir, self.frame_names[frame_idx]
                        )
                    )
                    ...
                    cv2.imwrite(
                        os.path.join(
                            self.save_tracking_results_dir,
        ...
        if save_all:
            if self.save_video:
                create_video_from_images(
                    self.save_tracking_results_dir, self.output_video_path
                )
    ...
    def update_files(
        ...
        text_prompt="object.",
        source_video_frame_dir="./tmp/source_video_frame",
        save_tracking_results_dir="./tmp/save_tracking_results",
    ):
    ...
    def save_mask(self, masks, frame_idx, save_dir=None):
        ...
        save_dir = (
            self.video_path.replace(PATH_SUFFIX, PATH_REPLACE)
            .replace(VIDEO_SUFFIX, "masks")
    ...
    def save_bbox(self, detections, frame_idx, save_dir=None):
        ...
        )  # Example image size, adjust as needed
        bbox_data = {"image_size": image_size}

        for idx, (bbox, class_id) in enumerate(
    ...
    def check_if_continue(self, images_dir):
        ...
        save_path = self.video_path.replace(PATH_SUFFIX, PATH_REPLACE).replace(
            VIDEO_SUFFIX, "masks.npy"
        )
        if os.path.exists(save_path):
            return False
        if os.path.exists(mask_dir) and os.path.exists(images_dir):
            if (
                self.check_dir_len(mask_dir) == self.check_dir_len(images_dir)
                and self.check_dir_len(mask_dir) != 0
            ):
                return True

        return False

    def generate_from_split_images_with_bbox(
        self,
        video_path,
        output_video_path="processed_video.mp4",
        text_prompt="object.",
        ...
        test_image = Image.open(
            os.path.join(source_video_frame_dir, self.frame_names[0])
        )
        image_size = test_image.size
        self.video_info = sv.VideoInfo(
            width=image_size[0], height=image_size[1], fps=25
        )
        # self.set_better_ref()
    ...
    def get_detections_with_bbox(
        ...
        if not self.input_boxes:
            ...
            self.load_converted_image()
            self.objects = [0] * len(self.input_boxes)
    ...
    def update_and_process(
        ...
        source_video_frame_dir = (
            video_path.replace(PATH_SUFFIX, PATH_REPLACE).replace(
                VIDEO_SUFFIX, "images"
            )
            if "0-th" not in video_path
            else source_video_frame_dir
        )
        print("save to ", source_video_frame_dir)
        if_continue = (
            self.check_if_continue(source_video_frame_dir)
            if split_for_metric is False
            else False
        )
        if if_continue:
        ...
        save_dir = self.video_path.replace(PATH_SUFFIX, PATH_REPLACE).replace(
            VIDEO_SUFFIX, "bbox"
        )
...
if __name__ == "__main__":
    ...
    for video in tqdm(filtered_videos):
        ...
        process_model.update_and_process(
            video_path,
            output_video_path="output_dir/" + str(cnt) + ".mp4",
            text_prompt="object.",
        )
